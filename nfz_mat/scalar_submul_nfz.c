...
void
nfz_mat_scalar_submul_nfz(nfz_mat_t B, const nfz_mat_t A, const nfz_t c,
			  const nfz_ctx_t ctx)
{
  fmpz_mat_struct * A_evl = _nfz_mat_eval_init(A->r, A->c, ctx);
  fmpz_mat_struct * B_evl = _nfz_mat_eval_init(A->r, A->c, ctx);
  ...
  for (long n = 0; n < ctx->deg; ++n)
    fmpz_mat_scalar_submul_fmpz(B_evl + n, A_evl + n, c_evl + n);
